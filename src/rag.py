...
_SYSTEM_PROMPT = (
    ...
    "回覆長度：每一輪回答盡量控制在 100 字以內。\n\n"
    "【思考與執行流程】\n"
    "請你在每次收到夥伴的問題時，在心裡依照以下步驟進行思考並給出回答：\n"
    "1. 判斷情境：分析這是在詢問行政與資源（如政大原資中心、文化活動、升學管道、住宿、獎學金與學雜費減免、校園生活支持、職涯發展等），還是單純的日常寒暄與生活支持？\n"
    "2. 決定行動：\n"
    "   - 若為行政與資源問題：必須呼叫 `isrc_knowledge_base` 工具檢索資料，若呼叫工具後有獲得相關資訊，請從中精確擷取關鍵段落，並轉化為口語、帶有溫度的回覆。\n"
    "   - 若為日常寒暄或工具找不到答案：請跳過資料檢索，直接以最親近、自然的語氣給予陪伴或安慰，絕對不可亂編。\n"
    ...
    def __init__(self, index: VectorStoreIndex | None = None, ctx=None):
        ...
        vector_tool = QueryEngineTool(
            ...
            metadata=ToolMetadata(
                ...
                description="當夥伴詢問有關如政大原資中心、文化活動、升學管道、住宿、獎學金與學雜費減免、校園生活支持、職涯發展等具體資源與行政問題時，必須使用此工具檢索知識庫！"
            )
        )
