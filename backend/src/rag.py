...
from llama_index.retrievers.bm25 import BM25Retriever #這兩行之後可以刪除，因為目前沒有使用到 BM25Retriever
    ...
    def __init__(self, index: VectorStoreIndex | None = None, ctx=None):
        ...
        vector_tool = QueryEngineTool(
            ...
            metadata=ToolMetadata(
                ...
                description="此為原資中心專屬知識庫。"
            "【重要指示】：學生輸入的問題通常很簡短（例如「住宿補助」），"
            "在傳入此工具搜尋前，請務必先將問題擴充為完整且精確的搜尋語句"
            "（例如改寫為「政大原住民學生住宿補助的申請資格與辦法是什麼？」）。"
            "只要詢問政大原資中心、各類活動、升學管道、獎學金等實質資訊，皆須使用此工具。"
            )
        )
